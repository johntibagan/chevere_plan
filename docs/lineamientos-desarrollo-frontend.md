...
- **UI responsiva:** grillas/tarjetas según ancho (`LayoutBuilder` / `MediaQuery` + `FeedLayout.childAspectRatioForWidth`). Portada `Expanded` + `BoxFit.cover`; textos ≈45% (portada ≈55%). CTA extra en pie compacto (restan a la portada). Regla: `.cursor/rules/ui-responsive.mdc`.
- **Tokens concretos:**
  | Qué | Dónde | Uso |
  |---|---|---|
