...
Paginación **en servidor**: **15** por página (`SearchPolicies.pageSize` → `p_limit` / `p_offset` en `search_sites`). «Cargar más» pide la siguiente página y la concatena; al cambiar filtros se reinicia el offset. Si la última página trae menos de 15, se oculta el botón. La fila de **conteo de resultados + Vista** (lista/2/3/4) queda **fija** debajo de las categorías (no se va al hacer scroll). Resultados: tuyos, públicos, catálogo o vinculados.
...
- Portal público (GitHub Pages): `https://johntibagan.github.io/chevere_plan/` — versión + descarga + reportes anónimos. Cada reporte tiene un consecutivo corto (`#1`, `#2`…) para commits (tocar copia). El dueño (PIN) puede marcar **en revisión**: el público ya no edita ni borra ese ítem. **Mejoras y reportes:** arriba pendientes y en revisión; resueltos en bloque colapsado (la partición se fija al cargar la página). Menú lateral con `#version`, `#reportes`, `#como-probar`. Ancho ~90% en escritorio; columna Descripción priorizada. **Cómo probar:** desplegables por versión; solo la versión actual del APK queda abierta al cargar (`beta_qa_flows`). Al decir **publica** en el chat, el agente pide los IDs y escribe esos flujos. Deploy de Pages: workflow `beta-portal-pages` en runner **self-hosted** (WSL); solo si cambia `beta-portal/`.
