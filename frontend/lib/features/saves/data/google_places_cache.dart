...
/// Caché simple de Place Details / reverse / nearby (TTL 14 días).
class GooglePlacesCache {
  ...
  Future<List<GeoPlace>?> readList(String key) async {
    final prefs = await SharedPreferences.getInstance();
    final raw = prefs.getString('$_prefix${_safe(key)}');
    if (raw == null || raw.isEmpty) return null;
    try {
  ...
  Future<void> writeList(String key, List<GeoPlace> places) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(
      '$_prefix${_safe(key)}',
      jsonEncode({
