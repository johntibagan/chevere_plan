-- Fix search_sites: ORDER BY distance_km era ambiguo vs columna OUT (PL/pgSQL).
-- Eso hacía fallar el RPC y la app caía al fallback (públicos a menudo vacíos).
...
begin
  ...
  with base as (
    select
      ...
      s.name as site_name,
      s.city as site_city,
      s.department as site_department,
      st_y(s.location::geometry) as site_lat,
      st_x(s.location::geometry) as site_lng,
      s.estimated_price_amount as site_price,
      s.currency_code::text as site_currency,
      exists (
  ...
  from base b
  order by
    case when b.site_distance_km is null then 1 else 0 end,
    b.site_distance_km nulls last,
    b.site_name
  limit 100;
end;
$$;
