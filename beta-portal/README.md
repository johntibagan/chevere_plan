...
| Agregar reporte (anónimo) | Este portal → tabla `beta_feedback` (sin prioridad hasta que el dueño la asigne) |
| Editar / borrar | Solo si **no** está en revisión ni listo |
| Prioridad (alta / media / baja) | Solo dueño con PIN (`beta_set_feedback_priority`); nuevos = `null` |
